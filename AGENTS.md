...
```bash
pnpm install
pnpm capture -- <url> --out .artifacts/site-capture --max-pages 40 --offline-qa
```
...
- Never run `pnpm exec playwright install chromium` unless the local capture fails with a missing-browser error.
